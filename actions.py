...
def say(words,language = 'en', voice='ty'):
    if language == 'vi':
        ...
        print(words)
        if (voice == 'be') :
            headers = {'api_key': '405c19610d4242efbc40710e6c4994d4','voice': 'hatieumai','speed':'2'}
        elif (voice == 'xuka') :
            headers = {'api_key': '405c19610d4242efbc40710e6c4994d4','voice': 'female','speed':'2'}
        else:
            headers = {'api_key': '405c19610d4242efbc40710e6c4994d4','voice': 'male','speed':'2', 'prosody':'1'}

        url = 'http://api.openfpt.vn/text2speech/v4'
        r = requests.post(url,headers=headers, data = words.encode('utf-8'))
        data = json.loads(r.text)
        if 'async' in data:
            ttsfilename = data['async'].replace('https','http')
            r = requests.get(ttsfilename)
            status_code = r.status_code
            while status_code == 404:
                status_code = requests.get(ttsfilename).status_code
            os.system("mpg321 " + ttsfilename)
    else:
        print('Speaking %s, %s' % (language, words))
...
# def synthesize_text(text, language_code='en-US'):
#     """Synthesizes speech from the input string of text."""
    ...
    #     audio_encoding=texttospeech_v1beta1.enums.AudioEncoding.MP3, speaking_rate=1, volume_gain_db=6)
    #
    # response = client.synthesize_speech(input_text, voice, audio_config)
    #
    # # The response's audio_content is binary.
    # with open('/tmp/say.mp3', 'wb') as out:
    #     out.write(response.audio_content)
    #     print('Audio content written to file say.mp3')
...
def youtube_search(query,region_code = 'VN'):
...
def YouTube_Autoplay(phrase, options = None):
    urllist=[]
    say("Getting autoplay links")
    if options is not None:
        fullurl,urlid = youtube_search(phrase,options)
    else:
        fullurl,urlid = youtube_search(phrase)
...
def YouTube_No_Autoplay(phrase, options = None):
    urllist=[]
